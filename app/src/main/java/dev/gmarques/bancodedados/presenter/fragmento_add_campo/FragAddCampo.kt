...
class FragAddCampo : Fragment() {
    ...
    private lateinit var viewModel: FragAddCampoViewModel
    private val args: FragAddCampoArgs by navArgs()

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?,
    ): View = FragAddCampoBinding.inflate(inflater)
        .also { binding = it }.root

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        viewModel = ViewModelProvider(this).get(FragAddCampoViewModel::class.java)
    ...
    private fun observarErrosDeEntrada() {
        viewModel.erroDeEntrada.observe(viewLifecycleOwner) {
            if (it != 0) Snackbar
                .make(binding.fabConcluir, getString(it), Snackbar.LENGTH_LONG)
                .show()
        }
    }
    ...
    private fun validarEntradaDoUsuario(): Boolean {
        ...
        if (!viewModel.validarNome(binding.edtNome.text?.toString()))
            return false
        ...
        if (binding.chipTexto.isChecked && !viewModel.validarRegrasTexto(compMax, compMin))
            return false

        val maiorQue = binding.edtMaiorQue.text.toString().toIntOrNull()
        val menorQue = binding.edtMenorQue.text.toString().toIntOrNull()

        if (binding.chipNumero.isChecked && !viewModel.validarRegrasNumero(maiorQue, menorQue))
            return false
